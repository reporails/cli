...
def format_compact(
    ...
    # L0: no instruction files — show level only, no score
    if result.level == Level.L0:
        return f"{level_label} ({level}) — no instruction files found"
...
def format_score(result: ValidationResult, _ascii_mode: bool | None = None) -> str:
    ...
    # L0: no instruction files — show level only, no score
    if result.level == Level.L0:
        return f"ails: {level_label} ({result.level.value}) — no instruction files found"
