...
- Internals: consolidated five subsystems into named subpackages — `core/funnel/`, `core/classify/`, `core/heal/`, `core/discovery/`, `core/lint/` — each matching its design-spec boundary
...
- Build: bundle the `en_core_web_sm` spaCy pipeline (~15 MB) inside the wheel under `bundled/spacy/`, alongside the existing bundled ONNX embedder. `core/mapper/models.py` loads the pipeline by local filesystem path via `get_spacy_model_path()`. End users no longer need a separate model download — `pip install reporails-cli` (or `uv pip install`) delivers the full model bundle. Replaces the prior runtime auto-download path that silently failed under `uv` virtualenvs.
...
- API client: outgoing diagnostic requests now carry a `User-Agent: reporails-cli/<version>` header for accurate attribution in server-side logs; previously the generic `python-httpx/<version>` default was sent. A regression test in `tests/unit/test_api_client.py::TestOutgoingHeaders` asserts the header is present on both anonymous and authenticated requests.
- Internals: cleaned up internal terminology references in `ails heal --help`, the MCP `heal` tool description, the `heading-as-instruction` rule prose, and module docstrings in `core/mapper/classify.py` and `core/mapper/serialize.py` (no behaviour change).
